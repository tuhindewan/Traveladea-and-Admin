...
$Blog = new Blog();
$blog_ids  = $Blog->getBlogIdByTagName($tag_name);
      ...
      <div class="container page">
        <div class="row masonry">
          <div class="col-md-12">
            ...
            <?php 
              while ( $data = $blog_ids->fetch_assoc()) {
               $blogs = $Blog->getBlogByIdForPagination($data['blog_id'],$pak_limit);
            ...
            <div style="text-align:center;">
              ...
              <nav class="pagination" role="navigation">
                <a class="prev" style="margin-right: 5px;" <?php if($_GET['page']== 1){?> id="not_work" <?php } ?> href="tag_blog_list.php?page=<?php echo ($_GET['page'] - 1); ?>"></a>
                <?php
                  for ($i=1; $i <= $total_page; $i++) { 
                ?>
                <a <?php if($_GET['page']== $i){?> class="active" <?php } ?> href="tag_blog_list.php?page=<?php echo $i; ?>"><?php echo $i; ?></a>
                <?php  }?>
                <a class="next" <?php if($_GET['page']== $total_page){?> id="not_work" <?php } ?> href="tag_blog_list.php?page=<?php echo ($_GET['page'] + 1); ?>"></a>
              </nav>
            </div>
          </div>
        </div>
