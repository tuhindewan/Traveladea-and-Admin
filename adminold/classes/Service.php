...
class Service
{
	...
	public function serviceEdit($data , $service_id){
			...
			if (empty($title) || empty($description) || empty($position) || empty($type)) {
				$msg = "<div class='alert alert-danger' role='alert'>All fields are required.</div>";
				return $msg;
			}else{

			$query = "UPDATE services SET status = '$status', title = '$title', description = '$description', position = '$position', type_name = '$type' WHERE service_id = $service_id ";
			$updateRow = $this->db->update($query);
			if ($updateRow) {
